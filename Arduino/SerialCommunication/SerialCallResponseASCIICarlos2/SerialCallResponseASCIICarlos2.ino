...
int sensorVal1 = 0;    // analog sensor
int sensorVal2 = 0;    // analog sensor
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    Serial.readBytesUntil('\r',dataBuffer,bufferSize);
    // get incoming byte:
    if(dataBuffer[0] == 'r') {
      // pin number
      int rpin1 = dataBuffer[1];
      int rpin2 = dataBuffer[2];
      // read analog input:
      sensorVal1 = analogRead(rpin1);
      sensorVal2 = analogRead(rpin2);
      // send sensor value(s):
      Serial.print(sensorVal1);
      Serial.print(",");
      Serial.println(sensorVal2);
    } else if(dataBuffer[0] == 'w') {
      // pin number
      int wpin = dataBuffer[1];
      int state = dataBuffer[2];
      pinMode(wpin, OUTPUT); // set pin to be an output
      digitalWrite(wpin, state);
      Serial.println(state);
    } else {
      Serial.println(-1);
    }
    memset(dataBuffer, 0, sizeof(dataBuffer));   // Clear contents of buffer
    serialFlush();
  }
}
